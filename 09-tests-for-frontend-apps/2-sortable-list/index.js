export default class SortableList {
    ...
    onPointerDown (event) {
        // Проверим что левая кнопка мыши
        if (event.which !== 1) { 
          return false;
        }
        ...
        if (itemElem) {
           // Если мы схватили за data-grab-handle требование к програме то включим драг дроп 
          ...
          // Если это строчка удаления то удаляем элемент 
    ...
    onDocumentPointerMove = ({ clientX, clientY}) => {
      this.moveDraggingAt(clientX, clientY);
      // Первый элемент массива, последний и остальные в детях
      const { firstElementChild, lastElementChild, children } = this.element;
      // коржинаты первого элемента минимальные
      const { top: firstElementTop } = firstElementChild.getBoundingClientRect();
      // кординаты второго элемента максимальные
      const { bottom: lastElementBottom } = lastElementChild.getBoundingClientRect();
  
      if (clientY < firstElementTop) {
        this.movePlaceholderAt(0); // если выше первого то всегда первый
      } else if (clientY > lastElementBottom) {
        this.movePlaceholderAt(children.length); // если ниже последнего то всегда последний
      } else {
        for (let i = 0; i < children.length; i++) {
          const li = children[i];
  
          // если элемент не сам то начнем
          if (li !== this.draggingElem) {
            const { top, bottom } = li.getBoundingClientRect();
            const { offsetHeight: height } = li;
  
            if (clientY > top && clientY < bottom) {
              // если наш элемент находится по высоте надд другим объектом то
              if (clientY < top + height / 2) {
                //ели над первой половиной, то подымаем
                this.movePlaceholderAt(i);
                break;
              } else {
                // если над нижней половино  то опустим
